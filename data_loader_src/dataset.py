...
# from data_loader_src.load_data import load_twitch, load_fb100
from data_loader_src.data_utils import rand_train_test_idx, even_quantile_labels, dataset_drive_url
...
def load_nc_dataset(dataname, sub_dataname=''):
    if dataname == 'twitch-e': # twitch-explicit graph
        ...
        dataset = load_wikics(dataname)
    elif dataname in ['Computers', 'Photo']:
        dataset = load_Amazon(dataname)
    elif dataname in ['cs', 'physics'] :
        dataset = load_coauthor(dataname)
...
def load_Amazon(dataname):
    graph = Amazon(root=parent_path + '/dataset/{}'.format(dataname), name='{}'.format(dataname)).data
    # .label = dataset.y

    dataset = NCDataset(dataname)
    dataset.graph = {'edge_index': graph.edge_index,
                     'node_feat': graph.x,
                     'edge_feat': None,
...
def load_wikics(dataname):
    graph = WikiCS(root=parent_path+'/dataset/{}'.format(dataname)).data
    dataset = NCDataset(dataname)
    runs = 20
    dataset.train_idx, dataset.valid_idx, dataset.test_idx = [], [], []
    for i in range(runs):
        dataset.train_idx.append(torch.where(graph.train_mask[:,i])[0].long())
        dataset.valid_idx.append(torch.where(graph.val_mask[:, i])[0].long())
    dataset.test_idx.append(torch.where(graph.test_mask)[0].long())
...
def load_coauthor(dataname):
    graph = Coauthor(root=parent_path+'/dataset/{}'.format(dataname), name=dataname).data
    dataset = NCDataset(dataname)
    dataset.graph = {'edge_index': graph.edge_index,
                     'node_feat': graph.x,
                     'edge_feat': None,
...
def load_Actor():
    graph = Actor(root=parent_path + '/dataset/{}'.format('Actor')).data
    ...
    for i in range(runs):
        dataset.train_idx.append(torch.where(graph.train_mask[:,i])[0].long())
        dataset.valid_idx.append(torch.where(graph.val_mask[:, i] )[0].long())
        dataset.test_idx.append( torch.where(graph.test_mask[:, i])[0].long())

    dataset.graph = {'edge_index': graph.edge_index,
                     'node_feat': graph.x,
                     'edge_feat': None,
                     'num_nodes': graph.x.shape[0]}
    label = torch.tensor(graph.y, dtype=torch.long) # [7600, 932]
    dataset.label = label
    return dataset


def load_webKB(dataname):
    graph = WebKB(root=parent_path + '/dataset/{}'.format(dataname), name='{}'.format(dataname)).data
    # .label = dataset.y
